...
class Booking extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($dtb < 1) {
            $this->session->set_flashdata('pesan', '<div class="alert alert-messege alert-danger" role="alert">Tidak Ada Buku dikeranjang!</div>');
            redirect(base_url());
        } else {
    ...
    public function tambahBooking()
    {
        ...
        //memilih data buku yang untuk dimasukkan ke table temp/keranjang melalui variabel $isi
        $d = $this->db->query("Select*from buku where id='$id_buku'")->row();

        //berupa data2 yang akan disimpan ke dalam table temp/keranjang
        $isi = [
            'id_buku' => $id_buku,
        ...
        if ($databooking > 0) {
            $this->session->set_flashdata('pesan', '<div class="alert alert-danger alert-messege" role="alert">Masih ada Booking Buku sebelumnya yang belum diambil.<br> Ambil Buku yang dibooking atau tunggu 1x24 jam untuk dapat booking kembali. </div>');
            redirect(base_url());
        }

        //jika buku yang diklik booking sudah ada di keranjang
        if ($temp > 0) {
            $this->session->set_flashdata('pesan', '<div class="alert alert-danger alert-messege" role="alert">Buku ini sudah Anda Booking!</div>');
            redirect(base_url() . 'home');
        }

        //jika buku yang akan dibooking sudah mencapai 3 item
        if ($tempuser == 3) {
            $this->session->set_flashdata('pesan', '<div class="alert alert-danger alert-messege" role="alert">Booking Buku tidak boleh lebih dari 3 buah.</div>');
            redirect(base_url() . 'home');
        }
        ...
        $this->session->set_flashdata('pesan', '<div class="alert alert-success alert-messege" role="alert">Buku berhasil ditambahkan ke keranjang </div>');
        redirect(base_url() . 'home');
    }
    ...
    {
        //mengupdate stok dan dibooking ditabel buku saat proses booking deselesaikan
    ...
    public function exportToPdf()
    {
        ...
        $mpdf->WriteHTML($html);
        $mpdf->Output("bukti-booking-$id_user.pdf", 'D');
        ...
        //$this->dompdf->render();
        //$this->dompdf->stream("bukti-booking-$id_user.pdf", array('Attachment' => 0)); //nama file yang dihasilkan
    }
}
